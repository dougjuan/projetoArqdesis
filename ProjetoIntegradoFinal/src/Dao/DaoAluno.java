...
public class DaoAluno {
	...
	public ToAluno carregar(int id) {
		ToAluno toAluno = new ToAluno();
		toAluno.setId(id);
		String sqlSelect = "SELECT nome, endereco,telefone,email,rg,cpf,login,senha FROM aluno WHERE cod_aluno = ?";
		// usando o try with resources do Java 7, que fecha o que abriu
		try (Connection conn = FabricaConexao.getConexao(); 
	...
	public ArrayList<ToAluno> listarAlunos(){
		ToAluno toAluno;
	...
	public ArrayList<ToAluno> listarAlunos(String chave){
		ToAluno toAluno;
		ArrayList<ToAluno> lista = new ArrayList<>();
